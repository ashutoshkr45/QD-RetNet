...
class MutualTrainer:
    ...
    def train_model1(self, epoch, train_loader, test_loader):
        ...
        for i, (inputs, labels_onehot, _) in enumerate(train_loader):
            ...
            # DML loss after epoch 10
            dml_loss = 0
            if epoch >= self.opts.dml_epoch:
                with torch.no_grad():
                    preds_s2, _ = self.model2_student(fundus_input)
                dml_loss = self.criterion_kl(F.log_softmax(preds_s1, dim=1),
                                          F.softmax(preds_s2.detach(), dim=1))
            
            # Total loss
            total_loss = cls_loss_student + kd_loss + (dml_loss if epoch >= self.opts.dml_epoch else 0)
    ...
    def train_model2(self, epoch, train_loader, test_loader):
        ...
        for i, (inputs, labels_onehot, _) in enumerate(train_loader):
            ...
            with torch.no_grad():
                preds_t2_d, feats_t2_d = self.model2_teacher(fundus_input)
            ...
            if epoch >= self.opts.dml_epoch:
                with torch.no_grad():
                    preds_s1, _ = self.model1_student(oct_input)
                dml_loss = self.criterion_kl(F.log_softmax(preds_s2, dim=1),
                                              F.softmax(preds_s1.detach(), dim=1))
