...
import importlib
import numpy as np
from models_qat import load_quant_separate_model_II
...
def parse_args():
    ...
    parser.add_argument("--model_id", default="q2", type=str, help="to make unique path for saving model")
...
def main(opts):
    ...
    model_o, model_f = load_quant_separate_model_II(configs, device, checkpoint)
    ...
    for epoch in range(configs.train_params["max_epoch"]):
        ...
        if test_metric > best_metric:
            ...
            model_f.eval()
            quantized_model = torch.ao.quantization.convert(model_f.cpu())
