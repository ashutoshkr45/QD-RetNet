...
import torch
import pickle
from .DataOrganizer import DataOrganizer_fo, DataOrganizer_of


class BaseDataLoader_I(object):
...
class SingleDataLoader(BaseDataLoader_I):
    def __init__(self, opts, configs):
        super(SingleDataLoader, self).__init__(opts, configs)
        self.get_data = DataOrganizer_fo.get_fundus_data
...
class OctDataLoader(SingleDataLoader):
    def __init__(self, opts, configs):
        ...
        self.get_data = DataOrganizer_fo.get_oct_data


class MultiDataLoader_I(BaseDataLoader_I):
    def __init__(self, opts, configs):
        super(MultiDataLoader_I, self).__init__(opts, configs)
        self.get_data = DataOrganizer_fo.get_mm_data
    ...
    def aa_get_camgenerating_dataloader(self):
        ...
        if moda == 'multi':
            ...
            dataset = DataOrganizer_fo.get_fundus_data(**camgenerating_params)
        else:
            dataset = DataOrganizer_fo.get_oct_data(**camgenerating_params)
        print("###########################################\ndataset statistics:")
        dataset.label_statistic()
        return torch.utils.data.DataLoader(dataset, batch_size=1, num_workers=self.opts.num_workers)
    ...
    def get_dataset_for_eval(collection):
        return DataOrganizer_fo.get_mm_data(collection=collection, if_eval=True)
    

class BaseDataLoader_II(object):
    def __init__(self, opts, configs):
        self.opts = opts
        self.configs = configs
...
class MultiDataLoader_II(BaseDataLoader_II):
    """DataLoader class for OCT Teacher -> Fundus Student setup"""
    def __init__(self, opts, configs):
        super(MultiDataLoader_II, self).__init__(opts, configs)
        self.get_data = DataOrganizer_of.get_mm_data
    ...
    def get_training_dataloader(self):
        ...
        trainset = self.get_data(**trainset_params)
        train_dataloader = torch.utils.data.DataLoader(
            trainset, batch_size=self.configs.train_params["batch_size"], 
            shuffle=True, num_workers=self.opts.num_workers)
        ...
        val_dataloader = torch.utils.data.DataLoader(
            valset, batch_size=1, num_workers=self.opts.num_workers)

        testset_params = self.init_testset_params()
        testset = self.get_data(**testset_params)
        test_dataloader = torch.utils.data.DataLoader(
            testset, batch_size=1, num_workers=self.opts.num_workers)
    ...
    def init_trainset_params(self):
        if not self.configs.if_syn:
            self.configs.syn_collection = None
        return {
            "collection": self.opts.train_collection,
            "aug_params": self.configs.aug_params["augmentation"],
            "transform": self.configs.transform,
            "if_test": False,
            "cls_num": self.configs.cls_num,
    ...
    def init_valset_params(self):
        return {
            "collection": self.opts.val_collection,
            "aug_params": self.configs.aug_params["onlyresize"],
            "transform": self.configs.transform,
            "if_test": False,
            "cls_num": self.configs.cls_num,
    ...
    def init_testset_params(self):
        return {
            "collection": self.opts.test_collection,
            "aug_params": self.configs.aug_params["onlyresize"],
            "transform": self.configs.transform,
            "if_test": False,
            "cls_num": self.configs.cls_num,
