...
class MultiDataset_fo(BaseDataset):
    """Multi-modal Dataset for Fundus Teacher -> OCT Student setup"""
    def __init__(
        ...
        super(MultiDataset_fo, self).__init__(aug_params, transform, if_test, cls_num)
        self.cls_num = cls_num
...
class MultiDataset_of(Dataset):
    """Multi-modal Dataset for OCT Teacher -> Fundus Student setup"""
    def __init__(self, img_f_path_list, img_o_path_list, labels_f_list=None,labels_o_list=None,aug_params=None, 
                 transform=None,if_test=False,cls_num=4,if_eval=False,queue_size=1):
        self.cls_num = cls_num
        self.queue_size = queue_size
        self.transform = transform
        self.aug_params = aug_params
        ...
        self.img_f_path_list = img_f_path_list  # Fundus images (student)
        self.img_o_path_list = img_o_path_list  # OCT images (teacher)
        ...
        for idx in range(len(self.labels_f_list)):
            ...
            self.class_list_f[class_id].append(img_f_path_list[idx])

        self.class_list_o = defaultdict(list)
        for idx in range(len(self.labels_o_list)):
            class_id = self._multi2binary(self.labels_o_list[idx])
            self.class_list_o[class_id].append(img_o_path_list[idx])

        self.eye_pairs = defaultdict(list)
        for img_o_path in self.img_o_path_list:
            img_o_filename = os.path.split(img_o_path)[-1]
            eye_id = '-'.join(img_o_filename.split('-')[0:2])
    ...
    def __getitem__(self, index):
        img_f_path = self.img_f_path_list[index]
        img_f_filename = os.path.split(img_f_path)[-1]
        img_f = (cv.imread(img_f_path) / 255.).astype(np.float32)

        label_onehot = np.array(self.labels_f_list[index])
        
        if not self.if_eval:
            # Get matching OCT image based on class
            if self._multi2binary(label_onehot) in self.class_list_o:
                img_o_path = random.choice(self.class_list_o[self._multi2binary(label_onehot)])
                label_onehot_o = label_onehot
            else:
                # If no matching class found, get random OCT image
                idx_r = random.randint(0, len(self.img_o_path_list) - 1)
                img_o_path = self.img_o_path_list[idx_r]
                label_onehot_o = self.labels_o_list[idx_r]

            # Get additional Fundus image from same class for consistency training
            img_f_path_add = random.choice(self.class_list_f[self._multi2binary(label_onehot)])
            label_onehot_o = np.array(label_onehot_o)
            img_o = (cv.imread(img_o_path) / 255.).astype(np.float32)
            img_f_add = (cv.imread(img_f_path_add) / 255.).astype(np.float32)

            # Apply augmentation if specified
            if self.aug_params:
                aug = augmentation.OurAug(self.aug_params)
                img_o = aug.process(img_o)
                img_f_add = aug.process(img_f_add)

            # Apply transforms if specified
            if self.transform:
                img_o = self.transform(img_o)
                img_f_add = self.transform(img_f_add)

        # Augment/transform the main Fundus image
        if self.aug_params:
            aug = augmentation.OurAug(self.aug_params)
            img_f = aug.process(img_f)
        ...
        if not self.if_eval:
            return (img_f, img_o, img_f_add), (label_onehot, label_onehot_o), img_f_filename
        else:
            return img_f, label_onehot, img_f_filename
